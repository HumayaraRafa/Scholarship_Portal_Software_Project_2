...
class IndexController extends Controller
{
    ...
    {
    	$scholarships=DB::table('scholarships')->where('removed',0)->orderby('scholarshipid','desc')
		    	->get();
    ...
    {
        $query="SELECT * FROM `scholarships` WHERE universityid in (SELECT universityid from universities where universityname LIKE '%".$req->universityName."%')";
    ...
    public function insertaccount(Request $req)
    {
        ...
        $userid= DB::table('users')->where('email',$req->email)->where('password',$req->password)->value('userid');
        $filename='CV_'.$req->fullname.'_'.time().'.pdf';
            $req->file('StudentCV')->storeAs('cv',$filename);
        ...
        DB::table('students')->insert([
            ...
            'state' => $req->state,
            'zip' => $req->zip,
            'prefferedDegree' => $req->prefferedDegree,
            'cv' => $filename
        ]);

        $req->session()->put('userid', $userid);
        $req->session()->put('status',0 );
        $req->session()->put('username', $req->qwert);
    ...
    {
        # code...
        DB::table('feedback')->insert(['value'=>$req->value]);
        $req->session()->put('feedback', '1');
        return redirect('/');
    }
